...
class map
{
  ...
  V& operator[](const K& key) {
      tree_node<K, V>* node = find(key, root_);
      if (node) {
          return node->value;
      }
  ...
  public:
      ...
      explicit iterator(tree_node<K, V>* n) : node(n) {}
      
      reference operator*() { return std::pair<K, V>(node->key, node->value); }
      pointer operator->() { return &std::pair<K, V>(node->key, node->value); }
  ...
  tree_node<K, V>* findMin(tree_node<K, V>* node) {
      if (node == nullptr) {
          return nullptr;
      }
  ...
  int getBalance(tree_node<K, V>* node) {
